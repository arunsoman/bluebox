<<<<<<< HEAD
// =============================================================================
// PipelineGraph3D — 3D force-directed graph of pipeline nodes using React Three Fiber
// =============================================================================
...
function buildEdgeGeometry(edges: GraphEdge[], nodeMap: Map<string, GraphNode>) {
...
function EdgeLines({
  ...
  const positions = useMemo(() => buildEdgeGeometry(edges, nodeMap), [edges, nodeMap]);
  const count = positions.length / 6; // each edge = 2 vertices * 3 coords
    ...
    <group position={node.position}>
      ...
      >
        ...
        <meshStandardMaterial
          color={color}
          emissive={color}
          emissiveIntensity={isSelected ? 0.6 : isHovered ? 0.3 : 0.1}
          roughness={0.4}
          metalness={0.6}
        />
      </mesh>
...
<<<<<<< HEAD
  ...
  const handlePointerOver = useCallback(
    (nodeId: string) => (e: ThreeEvent<PointerEvent>) => {
      e.stopPropagation();
  ...
  // Auto-center camera based on node positions
  const center = useMemo(() => {
    if (nodes.length === 0) return new THREE.Vector3(0, 0, 0);
    const avg = nodes.reduce(
      (acc, node) => acc.add(new THREE.Vector3(...node.position)),
