...
interface SettingsState {
  ...
  /* ── provider keys (in-memory only, not persisted) ── */
  pendingKeys: Record<string, string>; // provider_name -> key value being typed
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      /* ── set pending key (while typing) ── */
      setPendingKey: (provider: string, key: string) =>
        set((state) => ({
          pendingKeys: { ...state.pendingKeys, [provider]: key },
        })),
      ...
      submitKey: async (provider: string) => {
        ...
        try {
          ...
          // Refresh providers to show updated key status
