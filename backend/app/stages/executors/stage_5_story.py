...
class Stage5StoryExecutor(BaseStageExecutor):
    """Stage 5 — Story."""

    def __init__(self, llm_client: LLMClientInterface) -> None:
        super().__init__(stage_id=5, stage_name="story", llm_client=llm_client)

    async def build_prompt(self, blueprint: ProjectBlueprint, context: dict) -> str:
        return f"""Execute stage 5 (story) for the project.
