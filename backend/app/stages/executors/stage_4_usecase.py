...
class Stage4UsecaseExecutor(BaseStageExecutor):
    """Stage 4 — Usecase."""

    def __init__(self, llm_client: LLMClientInterface) -> None:
        super().__init__(stage_id=4, stage_name="usecase", llm_client=llm_client)

    async def build_prompt(self, blueprint: ProjectBlueprint, context: dict) -> str:
        return f"""Execute stage 4 (usecase) for the project.
