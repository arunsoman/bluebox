...
class Stage2ActorExecutor(BaseStageExecutor):
    """Stage 2 — Actor."""

    def __init__(self, llm_client: LLMClientInterface) -> None:
        super().__init__(stage_id=2, stage_name="actor", llm_client=llm_client)

    async def build_prompt(self, blueprint: ProjectBlueprint, context: dict) -> str:
        return f"""Execute stage 2 (actor) for the project.
