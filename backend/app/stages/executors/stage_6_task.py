...
class Stage6TaskExecutor(BaseStageExecutor):
    """Stage 6 — Task."""

    def __init__(self, llm_client: LLMClientInterface) -> None:
        super().__init__(stage_id=6, stage_name="task", llm_client=llm_client)

    async def build_prompt(self, blueprint: ProjectBlueprint, context: dict) -> str:
        return f"""Execute stage 6 (task) for the project.
